...
function scheduleCompounder(): void {
  ...
  setTimeout(() => {
    ...
    runStep("Step 5: Compounder", "compounder.ts");
    scheduleCompounder();
  }, msUntilMidnight);
}
...
function getDashboardHtml(): string {
  return [
    ...
    ".pipe { display:flex; gap:8px; align-items:center; margin-bottom:20px; }",
    ".step { flex:1; background:#0d0d1a; border:1px solid #1e2d4a; border-radius:8px; padding:12px; text-align:center; }",
    ".step.done { border-color:#16a34a; background:#0f2d1a; }",
    ".snum { font-size:18px; font-weight:700; margin-bottom:4px; }",
    ...
    "    <div class='arrow'>→</div>",
    "    <div class='step' id='p3'><div class='snum'>3</div><div class='sname'>Predict</div><div class='sstat' id='s3'>—</div></div>",
    "    <div class='arrow'>→</div>",
    "    <div class='step' id='p4'><div class='snum'>4</div><div class='sname'>Execute</div><div class='sstat' id='s4'>—</div></div>",
    "    <div class='arrow'>→</div>",
    "    <div class='step' id='p5'><div class='snum'>5</div><div class='sname'>Compound</div><div class='sstat' id='s5'>—</div></div>",
    "  </div>",
    "  <div class='g4'>",
    ...
    "function rPipe(){var keys=[data.scan,data.research,data.signals,data.execLog,data.kb];for(var i=0;i<5;i++){var el=document.getElementById('p'+(i+1));var st=document.getElementById('s'+(i+1));if(keys[i]){el.className='step done';st.textContent='Done';}else{el.className='step';st.textContent='Pending';}}}",
